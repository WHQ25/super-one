...
File-level touchpoints for each row: `experiences.md`. Minimum event set:
`event-contract.md`.
...
| You are… | Read |
|---|---|
| Adding a brand-new harness | `new-harness.md` — P0→P5 file lists. Host SuperOne tools are part of **P2**, not polish. |
| Closing one experience gap | `experiences.md` — do every cell in that row |
| Chat UI blank / wrong | `event-contract.md` |
...
Work top-down. `new-harness.md` is ordered so something is always runnable.
...
See `superone-tool` → `references/contract.md`, permission sections. Pass the turn `AbortSignal`; `allowAlwaysAllow: false` for
