...
describe('CodexSkillsRpcService.list', () => {
  it('maps SkillMetadata into SkillInfo and merges across cwd entries with scope→ResourceScope translation', async () => {
    ...
    expect(result).toEqual([
      { name: 'lint', displayName: 'lint', scope: 'user', description: 'Run lint', argumentHint: '', hasConfig: false },
      { name: 'review', displayName: 'PR Review', scope: 'project', description: 'Review repo PRs', argumentHint: '', hasConfig: false },
      { name: 'docs', displayName: 'docs', scope: 'user', description: 'Built-in docs', argumentHint: '', hasConfig: false, builtin: true },
    ])
  })
