...
import { attachDeviceGestureEvents } from './gesture-events'
...
describe('attachDeviceGestureEvents', () => {
  it('forwards finite macOS rotation deltas to the owning renderer', () => {
    const target = fakeWindow()
    attachDeviceGestureEvents(target.win, 'darwin')
  ...
  it('does not register native gesture handling on other platforms', () => {
    const target = fakeWindow()
    attachDeviceGestureEvents(target.win, 'win32')
