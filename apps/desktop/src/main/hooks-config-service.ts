...
import type { HookConfig, HookEntry, HookEventName, HookSavePayload, HookScope } from '@superone/shared/agent-types'
...
function pathFor(scope: HookScope, cwd: string): string {
...
export function listHooks(cwd: string): HookConfig[] {
  const result: HookConfig[] = []
  const scopes: HookScope[] = ['user', 'project', 'local']
  for (const scope of scopes) {
    const data = readJsonFile(pathFor(scope, cwd))
...
export function saveHook(cwd: string, payload: HookSavePayload, replaceId?: string): void {
  if (replaceId) {
    deleteHook(cwd, replaceId)
  }
  const filePath = pathFor(payload.scope, cwd)
...
export function deleteHook(cwd: string, id: string): void {
  const parsed = parseId(id)
  if (!parsed) return
  const { scope, event, gIdx, eIdx } = parsed
  const filePath = pathFor(scope, cwd)
  const data = readJsonFile(filePath)
  const hooks = getHooksMap(data)
  const groups = hooks[event]
