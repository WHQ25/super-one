/**
 * Turning the iOS Simulator's own accessibility dump into the shared tree shape.
 *
 * Only what is specific to this platform lives here: the guest's point space, how it
 * rotates against the framebuffer, and the shape the helper emits. Everything that
 * reads or compares the resulting tree — fingerprints, semantic gaps, merging OCR,
 * search — is platform-neutral and lives in `../device/tree.ts`.
 */
...
import type { NormalizedAccessibilityTree } from '../device/tree'

export type { NormalizedAccessibilityTree }
...
export interface IosSimulatorRawNode {
  ...
  value?: string
  /**
   * `AXPlaceholderValue` — the prompt an empty field draws, not anything a user typed.
   *
   * Carried separately because an empty `UITextField` answers `AXValue` with it, so
   * the two arrive indistinguishable unless both are asked for.
   */
  placeholder?: string
  identifier?: string
  enabled?: boolean
...
export function normalizeAccessibilityTree(
  ...
  const convert = (raw: IosSimulatorRawNode): DeviceUiNode => {
    ...
    // its own accessibility label meant that one.
    if (raw.label) node.label = raw.label
    else if (raw.placeholder) node.label = raw.placeholder
