...
| `collaboration` | Before setting `config.cwd` or `config.worktree` in `session_collab_request`; covers shared checkout (default for read-only review), same-repo isolation for implementers, cross-project launches |
