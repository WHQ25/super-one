...
import { builtInCaptureRoots, syncZoneRoot } from './media-output-paths'
 ...
 * asset dirs, the session sync zone, and the legacy generated-media and temp
 * capture roots that saved transcripts still link by absolute path.
 *
...
export function getMediaReadableRoots(): string[] {
  return getReadableAssetRoots([
    ...
    ...listWorktreePaths(),
    // Where every session artifact lives now (session-sync-zone.md §7).
    syncZoneRoot(),
    // Read-only: generations and captures written before the zone existed.
    mediaGenOutputRoot(),
    ...builtInCaptureRoots(),
