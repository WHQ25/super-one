...
interface AgentAPI {
  ...
  /** `sessionId` targets one session (a mosaic pane, a side chat); omit it for the project's active session. */
  setSessionSettings(projectPath: string, settings: { model?: string | null; effort?: SendMessageRequest['effort'] | null; mode?: string | null; agentPreset?: string | null }, sessionId?: string): Promise<void>
...
interface AppAPI {
  ...
  /** Create an empty file or directory under `parentRelPath` ('' = project root); refuses to overwrite. */
  createEntry(folderPath: string, parentRelPath: string, name: string, kind: FileEntryKind): Promise<FileOpResult>
  ...
  /**
   * Fold *this* window down into a mini session window (reversible). `steps` is the
   * choreography measured from the live shell — main plays the window edges over it.
   */
  convertWindowToMini(projectPath: string, sessionId: string, title?: string, steps?: WindowFoldStep[]): Promise<void>
  /** Grow a converted mini window back into the full app; no-op elsewhere. */
  restoreWindowFromMini(): Promise<void>
  /** Current converted-mini state of this window — main is authoritative across reloads. */
  getWindowMiniMode(): Promise<WindowMiniMode | null>
  onWindowMiniModeChanged(callback: (mode: WindowMiniMode | null) => void): () => void
  ...
  /** Open an ephemeral side chat forked off a live session. Never persisted. */
  startSideChat(request: SideChatStartRequest): Promise<SideChatStartResult>
  /** Dispose a side chat and best-effort delete its provider transcript. */
  closeSideChat(sessionId: string): Promise<boolean>
  getGitStatusFiles(folderPath: string): Promise<GitStatusFile[]>
  getGitLog(folderPath: string, query?: string): Promise<GitLogEntry[]>
  /** `@git` popup rows for a local folder / worktree or a remote project key. */
  listGitMentionRefs(folderPath: string, kind: GitMentionRefKind, query?: string): Promise<GitMentionRefsResult>
  /** What `@git` / `@gh` may offer for this folder (repo present, gh reachable). */
...
interface MiniAppAPI {
  ...
  confirmInstall(tempDir: string, projectDir?: string, preapprovedTools?: string[]): Promise<MiniAppInstallResult>
