...
const devicePorts = new Map<string, MessagePort>()
...
const environmentAPI = {
  ...
  // Four channels stay iOS-only: what Xcode has installed, DeviceKit artwork, making
  // a simulator, and probing for Xcode at all. Android has no equivalent of any, and
  // an empty implementation would only be a lie the panel then has to check for.
  ...
  onDeviceRotateGesture: (callback: (rotation: number) => void) => {
    ...
    ipcRenderer.on(AgentIpcChannels.ENVIRONMENT_DEVICE_ROTATE_GESTURE, handler)
    return () => {
      ipcRenderer.removeListener(AgentIpcChannels.ENVIRONMENT_DEVICE_ROTATE_GESTURE, handler)
    }
  },
  workspaceListDir: (
  ...
  listSessions: (
    ...
    >,
  listPinnedSessions: (connectionId: string) =>
    ipcRenderer.invoke(
      AgentIpcChannels.ENVIRONMENT_LIST_PINNED_SESSIONS,
      connectionId,
    ) as Promise<PinnedSessionEntry[]>,
  createSession: (
    connectionId: string,
...
const appAPI = {
  ...
  listComputerUseDisplays: () =>
    ipcRenderer.invoke(AgentIpcChannels.COMPUTER_USE_LIST_DISPLAYS) as Promise<
      ComputerUseDisplayInfo[]
    >,
  onComputerUseDisplaysChanged: (callback: () => void) => {
    const handler = (): void => callback()
    ipcRenderer.on(AgentIpcChannels.COMPUTER_USE_DISPLAYS_CHANGED, handler)
    return () => {
  ...
  listScheduledSends: () => ipcRenderer.invoke(AgentIpcChannels.SCHEDULED_SEND_LIST),
  setScheduledSend: (sessionId: string, patch: ScheduledSendPatch, init?: ScheduledSendSessionInit) =>
    ipcRenderer.invoke(AgentIpcChannels.SCHEDULED_SEND_SET, sessionId, patch, init),
