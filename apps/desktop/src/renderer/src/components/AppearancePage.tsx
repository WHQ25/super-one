...
const THEME_PREVIEW_SURFACE: Record<'light' | 'dark', { sidebar: string; main: string; border: string; row: string; pill: string; userBubble: string; text: string; textSoft: string }> = {
  ...
  dark: { sidebar: '#1c1c1c', main: '#242424', border: '#333333', row: '#2c2c2c', pill: '#2a2a2a', userBubble: '#3a3a3a', text: '#565656', textSoft: '#454545' },
}
...
function ThemeSwatch({ scheme }: { scheme: 'light' | 'dark' }) {
  ...
  return (
    <div className="flex h-full w-full" style={{ backgroundColor: c.main }}>
      <div className="flex h-full w-[34%] shrink-0 flex-col gap-1 border-r p-1" style={{ borderColor: c.border, backgroundColor: c.sidebar }}>
      ...
      <div className="relative flex-1">
        ...
        <div className="absolute inset-x-1.5 bottom-1.5 h-2 rounded-full border" style={{ backgroundColor: c.pill, borderColor: c.border }} />
      </div>
    </div>
  )
}
  ...
  if (mode === 'system') {
    return (
      <div className="relative h-full w-full overflow-hidden">
        <div className="absolute inset-y-0 left-0 w-1/2"><ThemeSwatch scheme="light" /></div>
        <div className="absolute inset-y-0 right-0 w-1/2"><ThemeSwatch scheme="dark" /></div>
      </div>
    )
  }
