...
describe('automation host focus guard', () => {
  ...
  it('aborts the named in-flight call when the main process cancels it', async () => {
    let handler: ((req: { callId: string; sessionId: string; op: string; input: unknown }) => Promise<void>) | null = null
    const sendAutomationResult = vi.fn()
    ;(window as { browserHost?: unknown }).browserHost = {
      onAutomationCall: (cb: typeof handler) => {
        handler = cb
    ...
    useBrowserStore.setState({ tabs: {}, captureRefs: {}, fullResolutionCaptureRefs: {}, automationCounts: {} })
    useBrowserStore.getState().ensure('browser-stuck', 'https://example.com', 'session-a')
    const executeJavaScript = vi.fn(async () => true)
    const unregister = registerBrowserWebview('browser-stuck', {
      executeJavaScript,
    ...
    try {
      const screenshot = handler!({ callId: 'shot', sessionId: 'session-a', op: 'screenshot', input: { tab: 'browser-stuck' } })
      await vi.waitFor(() => expect(executeJavaScript).toHaveBeenCalledTimes(1))
      await handler!({ callId: 'cancel-1', sessionId: 'session-a', op: 'cancel', input: { callId: 'shot' } })
      await screenshot

      expect(sendAutomationResult).toHaveBeenCalledTimes(1)
      expect(sendAutomationResult).toHaveBeenCalledWith('shot', false, undefined, 'Cancelled after the main process timed out')
      expect(useBrowserStore.getState().fullResolutionCaptureRefs['browser-stuck']).toBeUndefined()
      expect(useBrowserStore.getState().automationCounts['browser-stuck']).toBeUndefined()
    } finally {
      unregister()
      unmount()
    }
