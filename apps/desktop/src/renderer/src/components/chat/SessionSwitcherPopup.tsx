...
export function SessionSwitcherPopup({ scopeRef }: SessionSwitcherPopupProps) {
  ...
  const remoteIds = useChatStore((s) => {
    ...
    return s.remoteSessions[s.activeProject] ?? EMPTY_REMOTE_IDS
  })
...
function SessionList({ rows, selectedIndex }: { rows: SwitcherRow[]; selectedIndex: number }) {
  ...
  return (
    <div ref={containerRef} className="overflow-y-auto py-1 max-h-[calc(70vh-44px)]">
      {rows.map((row, idx) => (
        <SessionRow key={row.sessionId} row={row} idx={idx} isSelected={idx === selectedIndex} />
      ))}
    </div>
  )
}
...
function SessionStatusIcon({ status, lastEventAt, isUnseen, isAutomation, isWorktree, isRemote }: SessionStatusIconProps) {
  ...
  if (status === 'background') {
    return <SquareActivity className="size-3 shrink-0 animate-pulse text-muted-foreground" />
  }
  if (isUnseen) {
    return <CircleCheck className="size-3 shrink-0 text-green-600 dark:text-green-400" />
  }
  if (isAutomation) {
    return <CalendarClock className="size-3 shrink-0 text-muted-foreground" />
  }
  if (isWorktree) {
    return <GitFork className="size-3 shrink-0 text-muted-foreground" />
  }
  if (isRemote) {
