...
import { restoreSentDraft } from './restore-sent-draft'
...
it('restores the original document while preserving text typed after a refused send', () => {
  ...
  const restored = restoreSentDraft({ draftText: 'new words', draftJson: { type: 'doc', content: [{ type: 'paragraph', content: [{ type: 'text', text: 'new words' }] }] }, attachments: [] }, saved)
  ...
  expect(restoreSentDraft({ draftText: '', draftJson: null, attachments: saved.attachments }, saved).attachments).toHaveLength(1)
})
