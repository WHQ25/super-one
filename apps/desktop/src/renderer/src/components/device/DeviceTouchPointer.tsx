...
import { normalizeFramePoint, unrotateFrameSize } from './device-input'
import type { DeviceInputApi } from './use-device-input'

type CanvasHandlers = DeviceInputApi['canvasHandlers'] & {
...
export interface DeviceTouchPointerApi {
 ...
 */
export function useDeviceTouchPointer({
  enabled,
  rotationDegrees,
  handlers,
  canvas,
 ...
 */
export function DeviceTouchPointer({ ref }: { ref: RefObject<HTMLDivElement | null> }) {
  return (
    <div
      ref={ref}
      aria-hidden
      data-device-touch-pointer=""
      data-state="idle"
