...
const STORAGE_KEY = 'superone.device.previewQuality'
const LEGACY_STORAGE_KEY = 'superone.iosSimulator.previewQuality'
...
export function readPreviewQuality(
  ...
  try {
    const parsed: unknown = JSON.parse(
      storage?.getItem(STORAGE_KEY) ?? storage?.getItem(LEGACY_STORAGE_KEY) ?? 'null',
    )
