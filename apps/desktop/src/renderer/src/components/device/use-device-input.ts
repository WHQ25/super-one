...
type SimulatorInput = Parameters<typeof window.environment.deviceInput>[1]
...
export interface DeviceInputApi {
...
export function useDeviceInput(
  ...
  const touchTracker = useRef(new DeviceTouchTracker())
  const syntheticGesture = useRef(new DeviceSyntheticGesture())
  ...
  const scheduleTouchMove = useCallback((contacts: DeviceTouchContact[]) => {
  ...
  const dispatchSyntheticFrames = useCallback(
    (frames: ReturnType<DeviceSyntheticGesture['scroll']>) => {
  ...
  useEffect(() => {
    if (!enabled) return
    const subscribe = window.environment.onDeviceRotateGesture
    if (!subscribe) return
