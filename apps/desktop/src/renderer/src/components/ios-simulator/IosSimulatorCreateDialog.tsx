...
import { messageOf } from './ios-simulator-report'
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{t('activity.iosSimulator.picker.create')}</DialogTitle>
        </DialogHeader>

        {runtimes === null ? (
          <div className="flex items-center justify-center gap-2 py-8 text-sm text-muted-foreground">
            <Loader2 data-icon className="animate-spin" />
            {t('activity.iosSimulator.checking')}
          </div>
        ) : runtimes.length === 0 ? (
          <p className="py-6 text-center text-sm text-muted-foreground">
            {t('activity.iosSimulator.picker.createEmpty')}
          </p>
        ) : (
          <div className="flex flex-col gap-3">
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="ios-simulator-runtime">{t('activity.iosSimulator.picker.stepVersion')}</Label>
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="ios-simulator-device-type">{t('activity.iosSimulator.picker.stepModel')}</Label>
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="ios-simulator-name">{t('activity.iosSimulator.picker.createName')}</Label>
              <Input
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={creating}>
            {t('activity.iosSimulator.picker.cancel')}
          </Button>
          <Button onClick={() => { void create() }} disabled={creating || !deviceType || !name.trim()}>
            {creating && <Loader2 data-icon className="animate-spin" />}
            {creating ? t('activity.iosSimulator.picker.creating') : t('activity.iosSimulator.picker.createConfirm')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
