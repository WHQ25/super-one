...
vi.mock('./IosSimulatorCreateDialog', () => ({
...
const MAX_26 = device({ udid: 'max-26', name: 'iPhone 17 Pro Max' })
const MAX_18 = device({
  udid: 'max-18',
...
const SE = device({ udid: 'se-26', name: 'iPhone SE (3rd generation)' })
const IPAD = device({
  udid: 'ipad-26',
  name: 'iPad Pro 13-inch (M4)',
  runtimeIdentifier: 'com.apple.CoreSimulator.SimRuntime.iOS-26-0',
  runtimeName: 'iPadOS 26.0',
  booted: true,
  state: 'Booted',
})
const TAKEN = device({ udid: 'taken-26', name: 'iPhone Air', boundSessionId: 'session-2' })

const DEVICES = [MAX_26, MAX_18, SE, IPAD, TAKEN]

function renderMenu(overrides: Partial<React.ComponentProps<typeof IosSimulatorDeviceMenu>> = {}) {
...
describe('iOS Simulator device menu', () => {
  ...
  it('picks a runtime from the model submenu and remembers it as recent', async () => {
    ...
    expect(onSelect).toHaveBeenCalledWith('max-18')
    expect(JSON.parse(localStorage.getItem('superone.iosSimulator.recentUdids') ?? '[]')).toEqual(['max-18'])
  })
  ...
  it('flattens a model that exists on exactly one runtime', async () => {
    ...
    expect(onSelect).toHaveBeenCalledWith('se-26')
  })

  it('lists a booted simulator under Running Now and keeps it out of Recent', async () => {
    localStorage.setItem('superone.iosSimulator.recentUdids', JSON.stringify(['ipad-26', 'se-26']))
    const user = userEvent.setup()
    renderMenu()
  ...
  it('hands a freshly created simulator to the panel in a single pass', async () => {
    const user = userEvent.setup()
    const { onSelect } = renderMenu()
    const fresh = device({ udid: 'new-26', name: 'iPhone 17' })
    ...
    expect(onSelect).toHaveBeenCalledWith('new-26')
    expect(JSON.parse(localStorage.getItem('superone.iosSimulator.recentUdids') ?? '[]'))
      .toEqual(['new-26'])
  })
