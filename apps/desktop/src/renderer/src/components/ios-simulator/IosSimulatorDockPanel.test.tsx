...
import {
  IOS_SIMULATOR_MIN_PANEL_WIDTH,
  IosSimulatorDockPanel,
  resolveIosSimulatorMinWidth,
} from './IosSimulatorDockPanel'

vi.mock('./IosSimulatorPanel', () => ({
  IosSimulatorPanel: () => <div data-testid="panel" />,
}))
...
describe('resolveIosSimulatorMinWidth', () => {
  it('asks for the full floor when the dock can spare it', () => {
    expect(resolveIosSimulatorMinWidth(1200)).toBe(IOS_SIMULATOR_MIN_PANEL_WIDTH)
  })

  it('never asks for more than the dock has', () => {
    // Demanding more is what makes dockview overflow the panel and clip the device.
    expect(resolveIosSimulatorMinWidth(280)).toBe(280)
  })

  it('leaves sibling groups their own floor', () => {
    expect(resolveIosSimulatorMinWidth(420, 1)).toBe(320)
  })

  it('falls back to dockview\'s default when the dock has not been measured', () => {
    expect(resolveIosSimulatorMinWidth(0)).toBe(DOCKVIEW_DEFAULT)
    expect(resolveIosSimulatorMinWidth(Number.NaN)).toBe(DOCKVIEW_DEFAULT)
  })
})
...
describe('iOS Simulator dock panel width floor', () => {
  it('holds the group above the width the device needs', () => {
    ...
    expect(a.api.setConstraints).toHaveBeenCalledWith({
      minimumWidth: IOS_SIMULATOR_MIN_PANEL_WIDTH,
    })
  })
