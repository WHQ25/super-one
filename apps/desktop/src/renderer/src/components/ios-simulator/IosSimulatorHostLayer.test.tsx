...
import { IosSimulatorHostLayer } from './IosSimulatorHostLayer'
 ...
 * the only thing in here that mounts an `IosSimulatorPanel`. That is the subject.
 */
function Surfaces() {
  ...
  return (
    <>
      ...
      {activityShown && (
        <div data-fake-dock="">
          <IosSimulatorView sessionId={SESSION_ID} mode="panel" />
        </div>
      )}
    </>
  )
...
function AppShape({ view }: { view: 'main' | 'settings' }) {
  if (view === 'settings') {
    return <><div data-settings="" /><IosSimulatorHostLayer /></>
  }
  return <><div data-main="" /><IosSimulatorHostLayer /></>
}

const host = () =>
  document.querySelector<HTMLElement>(`[data-ios-simulator-host][data-session-id="${SESSION_ID}"]`)
...
function presentation(): string | null {
  ...
  return element.style.opacity === '0'
    ? 'parked'
    : element.getAttribute('data-ios-simulator-presentation')
}

async function renderReady(node: React.ReactElement = <IosSimulatorHostLayer />) {
  mountChatRoot()
...
beforeEach(() => {
  ...
  Element.prototype.getBoundingClientRect = function (this: Element) {
    const mode = this.getAttribute('data-ios-simulator-slot')
...
describe('iOS Simulator host layer placement', () => {
  ...
  it('keeps the panel alive through the beat when no surface holds a slot', async () => {
    ...
    expect(useIosSimulatorPipStore.getState().slots[SESSION_ID]).toBeUndefined()
  ...
  it('does not paint into the dock rect while the activity panel is hidden', async () => {
    ...
    act(() => {
      useIosSimulatorPipStore.getState().updateSlot(SESSION_ID, 'panel', RECT)
...
describe('iOS Simulator expanded overlay', () => {
  ...
  it('paints the card under the device and the buttons over it', async () => {
    ...
    const layer = document.querySelector('[data-ios-simulator-host-layer]')!
    ...
    expect(at('[data-ios-simulator-host]')).toBeGreaterThan(at('.bg-background'))
    // ...and a wide device would otherwise swallow shrink and hide.
    expect(at('[data-device-preview-actions]')).toBeGreaterThan(at('[data-ios-simulator-host]'))
  })
