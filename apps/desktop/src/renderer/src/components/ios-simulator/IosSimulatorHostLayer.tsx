...
import {
  selectHostedIosSimulatorSessions,
  useIosSimulatorPipStore,
  type IosSimulatorSlot,
} from '@/stores/ios-simulator-pip'
import { useSashResizing } from '@/hooks/useSashResizing'
import { useGlobalDragging } from '@/hooks/useGlobalDragging'
import { IosSimulatorPanel } from './IosSimulatorPanel'
import { IosSimulatorOverlaySurface } from './IosSimulatorOverlaySurface'
import { IosSimulatorPictureInPicture } from './IosSimulatorPictureInPicture'
import { useIosSimulatorHandover } from './use-ios-simulator-handover'
 ...
 *
 * Exactly one `IosSimulatorPanel` per session with a device, mounted here for as long
 ...
 * `IosSimulatorView`), so switching between them repositions a running panel instead
 ...
 * See `IosSimulatorOverlaySurface`.
 */
export function IosSimulatorHostLayer() {
  const sessionIds = useIosSimulatorPipStore(useShallow(selectHostedIosSimulatorSessions))
  ...
  const overlayOpen = useIosSimulatorPipStore((state) => state.expandedSessionId != null)
  ...
  useIosSimulatorHandover()

  return (
    <div
      data-ios-simulator-host-layer=""
      style={{
        position: 'fixed',
        inset: 0,
        pointerEvents: 'none',
    ...
    >
      <IosSimulatorOverlaySurface />
      {sessionIds.map((sessionId) => (
        <PersistentIosSimulator key={sessionId} sessionId={sessionId} yielding={yielding} />
      ))}
...
function PersistentIosSimulator({ sessionId, yielding }: { sessionId: string; yielding: boolean }) {
  const panelSlot = useIosSimulatorPipStore((state) => state.slots[sessionId])
  const pipSlot = useIosSimulatorPipStore((state) => state.pipSlots[sessionId])
  const overlaySlot = useIosSimulatorPipStore((state) => state.overlaySlots[sessionId])
  const expanded = useIosSimulatorPipStore((state) => state.expandedSessionId === sessionId)
   ...
   */
  const slot: IosSimulatorSlot | undefined = activityShown
    ? panelSlot
    : expanded
      ? overlaySlot
      : pipSlot
  ...
  return (
    <div
      data-ios-simulator-host=""
      data-session-id={sessionId}
      data-ios-simulator-presentation={mode}
      style={{
        position: 'absolute',
