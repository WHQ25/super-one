...
describe('iOS Simulator synthetic gestures', () => {
  it('maps ordinary wheel movement to an inverse single-finger drag', () => {
    const gesture = new IosSimulatorSyntheticGesture()
  ...
  it('classifies trackpad and mouse wheel modifiers without stealing normal scroll', () => {
    expect(classifyIosSimulatorWheelGesture({ ctrlKey: false, altKey: false, shiftKey: false })).toBe('scroll')
    expect(classifyIosSimulatorWheelGesture({ ctrlKey: true, altKey: false, shiftKey: false })).toBe('pinch')
    expect(classifyIosSimulatorWheelGesture({ ctrlKey: false, altKey: true, shiftKey: false })).toBe('pinch')
    expect(classifyIosSimulatorWheelGesture({ ctrlKey: false, altKey: true, shiftKey: true })).toBe('rotate')
  })
})
