...
type GestureFrames = IosSimulatorTouchContact[][]
...
function moved(contact: Omit<IosSimulatorTouchContact, 'phase'>): IosSimulatorTouchContact {
  return { ...contact, phase: 'moved' }
}

export function classifyIosSimulatorWheelGesture(
 ...
 * Pointer-driven touches stay in IosSimulatorTouchTracker; callers must keep the
 * two trackers mutually exclusive while one owns the native contact slots.
 */
export class IosSimulatorSyntheticGesture {
  ...
  end(): IosSimulatorTouchContact[] | null { return this.finish('ended') }

  cancel(): IosSimulatorTouchContact[] | null { return this.finish('cancelled') }

  /** Releases the contacts the active mode is holding. Null when there are none. */
  private finish(
    phase: Extract<IosSimulatorTouchPhase, 'ended' | 'cancelled'>,
  ): IosSimulatorTouchContact[] | null {
  ...
  private transformContacts(
    phase: Extract<IosSimulatorTouchContact['phase'], 'began' | 'moved' | 'ended' | 'cancelled'>,
  ): IosSimulatorTouchContact[] {
    const { xScale, yScale } = this.transformScales()
