...
describe('ios simulator pip layout', () => {
  it('opens portrait in the top-right corner, inside the chat', () => {
    const layout = createDefaultIosSimulatorPipLayout(CHAT)

    expect(layout.width / layout.height).toBeCloseTo(IOS_SIMULATOR_PIP_ASPECT, 5)
  ...
  it('keeps the phone aspect instead of widening when dragged past the edge', () => {
    const start = createDefaultIosSimulatorPipLayout(CHAT)
    const shoved = clampIosSimulatorPipLayout({ ...start, left: 5_000, top: 5_000 }, CHAT)

    expect(shoved.width / shoved.height).toBeCloseTo(IOS_SIMULATOR_PIP_ASPECT, 5)
  ...
  it('fits a portrait box into a chat too short for the default height', () => {
    ...
    const layout = createDefaultIosSimulatorPipLayout(shortChat)
  ...
  it('takes the box from the device the caller measured', () => {
    const aspect = iosSimulatorPipAspect({ width: 1320, height: 2868 })
    const layout = createDefaultIosSimulatorPipLayout(CHAT, aspect)
  ...
  it('falls back to a phone when the device has not reported its framebuffer', () => {
    expect(iosSimulatorPipAspect(null)).toBe(IOS_SIMULATOR_PIP_ASPECT)
    expect(iosSimulatorPipAspect({ width: 0, height: 0 })).toBe(IOS_SIMULATOR_PIP_ASPECT)
  })
  ...
  it('takes the box from the device body, not the glass inside it', () => {
    ...
    const aspect = iosSimulatorPipAspect(SCREEN, IPHONE_17_PRO_MAX_CHROME)
  ...
  it('turns the body with the device', () => {
    const landscape = iosSimulatorPipAspect(
  ...
  it('falls back to the glass for a model that ships no artwork', () => {
    // Every iPad, every Apple TV, iPhone 11-14, SE. `IosSimulatorBareScreen` draws the
    // screen and nothing else there, so the framebuffer's own shape is exactly right.
    expect(iosSimulatorPipAspect(SCREEN, null)).toBeCloseTo(1320 / 2868, 5)
    expect(iosSimulatorPipAspect(SCREEN)).toBeCloseTo(1320 / 2868, 5)
  })
  ...
  it('lays a turned device down inside the chat instead of overflowing it', () => {
    ...
    const landscape = iosSimulatorPipAspect({ width: 2868, height: 1320 })
    const portrait = createDefaultIosSimulatorPipLayout(CHAT)
    const turned = clampIosSimulatorPipLayout(portrait, CHAT, landscape)
