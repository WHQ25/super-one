...
import { iosSimulatorOuterBox } from './ios-simulator-chrome-layout'
 ...
 */
export const IOS_SIMULATOR_PIP_ASPECT = 1320 / 2868

const IOS_SIMULATOR_PIP_DIMENSIONS: PipDimensions = {
  margin: 12,
  defaultWidth: 220,
  defaultHeight: 440,
  minWidth: 130,
 ...
 *
 * No chrome means no artwork for this model, and `IosSimulatorBareScreen` draws the
 * glass alone — for which the framebuffer's own shape is exactly right.
 */
export function iosSimulatorPipAspect(
  viewport: { width: number; height: number } | null | undefined,
  chrome?: IosSimulatorChrome | null,
): number {
  const screen = pipAspectOf(viewport ?? { width: 0, height: 0 }, IOS_SIMULATOR_PIP_ASPECT)
...
export function defaultIosSimulatorPipMaxHeight(bounds: PipBounds): number {
  return defaultPipMaxHeight(bounds, IOS_SIMULATOR_PIP_DIMENSIONS)
}

export function clampIosSimulatorPipLayout(
  layout: PipLayout,
  bounds: PipBounds,
  aspect: number = IOS_SIMULATOR_PIP_ASPECT,
  options?: { maxHeight?: number },
): PipLayout {
  return clampPipLayout(layout, bounds, IOS_SIMULATOR_PIP_DIMENSIONS, aspect, options)
}

export function createDefaultIosSimulatorPipLayout(
  bounds: PipBounds,
  aspect: number = IOS_SIMULATOR_PIP_ASPECT,
): PipLayout {
  return createDefaultPipLayout(bounds, IOS_SIMULATOR_PIP_DIMENSIONS, aspect)
}
