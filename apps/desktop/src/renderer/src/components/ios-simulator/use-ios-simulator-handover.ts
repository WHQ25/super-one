 ...
 */
export function useIosSimulatorHandover(): void {
  const { t } = useTranslation()
  const openTabLabel = t('activity.iosSimulator.title')
  ...
  const { sessionId, deviceOnScreen } = useIosSimulatorPreview()
  ...
  useEffect(() => {
    ...
    return window.environment.onIosSimulatorSessionState(currentSessionId, (state) => {
      ...
      store.setReady(currentSessionId, { udid: bound.udid, width, height })
      ...
      if (arriving && useActivityPanelStore.getState().showPanel) {
        revealIosSimulatorTab(currentSessionId, openTabLabel)
      }
    })
  ...
  useEffect(() => {
    ...
    if (deviceOnScreen && sessionId) revealIosSimulatorTab(sessionId, openTabLabel)
