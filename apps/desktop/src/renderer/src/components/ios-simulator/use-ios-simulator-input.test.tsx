...
const iosSimulatorInput = vi.fn(async () => ({ ok: true }))

beforeEach(() => {
  iosSimulatorInput.mockClear()
  Object.defineProperty(window, 'environment', {
    configurable: true,
    value: { iosSimulatorInput },
  })
})

function mountKeyboard() {
  const { result } = renderHook(() => useIosSimulatorInput({
    sessionId: 'session-a',
    enabled: true,
    canvas: null,
  }))
...
describe('useIosSimulatorInput keyboard', () => {
  it('leaves the pre-edit alone while an IME is composing', () => {
    ...
    expect(iosSimulatorInput).not.toHaveBeenCalled()
  })
  ...
  it('sends the committed string once and empties the box', () => {
    ...
    expect(iosSimulatorInput).toHaveBeenCalledTimes(1)
    expect(iosSimulatorInput).toHaveBeenCalledWith('session-a', { type: 'text', text: '你好' })
  ...
  it('forwards a typed character from the input event, not the keystroke', () => {
    ...
    expect(iosSimulatorInput).toHaveBeenCalledExactlyOnceWith('session-a', { type: 'text', text: 'a' })
    expect(box.value).toBe('')
  })
  ...
  it('does not forward the composed text a second time from the input event', () => {
    ...
    expect(iosSimulatorInput).toHaveBeenCalledExactlyOnceWith('session-a', { type: 'text', text: '你好' })
    expect(box.value).toBe('')
  })
  ...
  it('sends keys that carry no text of their own from the keystroke', () => {
    ...
    expect(iosSimulatorInput).toHaveBeenCalledExactlyOnceWith('session-a', { type: 'text', text: '\n' })
  })
