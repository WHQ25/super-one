...
describe('activity panel side swap', () => {
  // jsdom has no layout engine, so each element replays a scripted [before, after]
  // pair — one read before the commit, one after.
  const mount = (attr: string, lefts: [number, number]) => {
    const el = document.createElement('div')
    el.setAttribute(attr, '')
    const reads = [...lefts]
    el.getBoundingClientRect = () => ({ left: reads.shift() ?? lefts[1] }) as DOMRect
  ...
  it('inverts the order-flip jump on both panels so the swap animates', () => {
    ...
    const ap = mount('data-activity-outer', [0, 600])
  ...
  it('skips the animation for panels that did not move', () => {
    const ap = mount('data-activity-outer', [120, 120])
