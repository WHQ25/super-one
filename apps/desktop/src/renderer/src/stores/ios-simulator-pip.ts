/**
 * Where each session's simulator picture should be drawn, and which surface wins.
 *
 * The picture itself is not here and is not in the React tree that shows it: exactly
 * one `IosSimulatorPanel` per session lives permanently in `IosSimulatorHostLayer`,
...
export type IosSimulatorSlotMode = 'panel' | 'pip' | 'overlay'

/** Where a surface is, in viewport coordinates, for the host layer to sit on. */
export interface IosSimulatorSlot {
...
interface IosSimulatorPipState {
  ...
  slots: Record<string, IosSimulatorSlot>
  pipSlots: Record<string, IosSimulatorSlot>
  overlaySlots: Record<string, IosSimulatorSlot>
  ...
  updateSlot: (sessionId: string, mode: IosSimulatorSlotMode, rect: DOMRectReadOnly) => void
  unregisterSlot: (sessionId: string, mode: IosSimulatorSlotMode) => void
...
function slotsFor(
  state: Pick<IosSimulatorPipState, 'slots' | 'pipSlots' | 'overlaySlots'>,
  mode: IosSimulatorSlotMode,
): Record<string, IosSimulatorSlot> {
  return mode === 'pip' ? state.pipSlots : mode === 'overlay' ? state.overlaySlots : state.slots
}

function withSlots(
  mode: IosSimulatorSlotMode,
  next: Record<string, IosSimulatorSlot>,
): Partial<IosSimulatorPipState> {
 ...
 * costs real CPU.
 */
export function selectHostedIosSimulatorSessions(state: IosSimulatorPipState): string[] {
