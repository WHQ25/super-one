...
describe('loadInlineImage', () => {
  it('over the LAN downloads straight away and returns a data URI', async () => {
    ...
    expect(command).toMatchObject({ type: 'read_desktop_file', path: '/proj/.superone/shot.png', maxBytes: INLINE_IMAGE_MAX_BYTES, sessionId: 's1' })
    expect(command.statOnly).toBeUndefined()
    expect(h.downloadDesktopFile).toHaveBeenCalledWith(FILE)
  })

  it('over the relay asks for confirmation with the size, without moving bytes', async () => {
    const h = host(() => ({ ok: true, statOnly: true, name: 'shot.png', mimeType: 'image/png', size: 204_800, modifiedAt: 1 }))
    const result = await loadInlineImage({ ...base, transport: 'relay', host: h })
    expect(result).toEqual({ confirmRequired: true, size: 204_800 })
    const command = h.request.mock.calls[0][0] as Extract<RemoteCommand, { type: 'read_desktop_file' }>
