...
export function AgentConfigFields({ harness, config, profile: suppliedProfile, loadSystemInfo, onChange }: { harness: HarnessId; config: SessionAgentLaunchConfig; profile?: SessionAgentProfile; loadSystemInfo?: (harness: HarnessId) => Promise<RemoteSystemInfo>; onChange: (patch: Partial<SessionAgentLaunchConfig>) => void }) {
  ...
  useEffect(() => {
    ...
    }).catch((cause: unknown) => { if (active) setError(cause instanceof Error ? cause.message : t('Could not load agent options')) })
    return () => { active = false }
  ...
  return <View style={styles.stack}>
    ...
    {fast ? <EditableField field={{ label: t('Fast mode'), type: 'boolean' }} value={config.fastMode} onChange={(value) => onChange({ fastMode: value === true })} /> : null}
    ...
    {options.sandboxModes.length ? select('Sandbox', config.sandboxMode ?? 'off', options.sandboxModes, (sandboxMode) => onChange({ sandboxMode: sandboxMode as SessionAgentLaunchConfig['sandboxMode'] })) : <Text style={styles.meta}>{t('Execution isolation follows the agent’s permission settings.')}</Text>}
  </View>
}
