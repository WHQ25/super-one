...
export function parseNativeDiff(diff: string, tokens?: PermissionRequest['toolDiffTokens']): NativeDiffLine[] {
  ...
  for (const row of diff.split('\n')) {
    ...
    result.push({ kind, text, line: kind === 'removed' ? oldLine : newLine, tokens: source?.map(([part]) => part).join('') === text ? source : undefined })
