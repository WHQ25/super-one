...
export function AnchoredMenu(props: AnchoredMenuProps) {
  ...
  }, [host, id, props.anchor, props.title, props.onDismiss, props.children, props.width])
  useEffect(() => () => host.hide(id), [host, id])
  return null
}

function MenuSurface({ anchor, title, onDismiss, children, width = 300 }: AnchoredMenuProps) {
  ...
  return <View style={{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0 }} accessibilityViewIsModal onAccessibilityEscape={onDismiss}>
      ...
      <View style={{ position: 'absolute', ...layout, borderWidth: 1, borderColor: colors.border,
        ...
        <ScrollView keyboardShouldPersistTaps="always" bounces={false}
          ...
          <Text ref={titleRef} accessible accessibilityRole="header" style={{ paddingHorizontal: 8, paddingVertical: 8, fontSize: 12, color: colors.mutedForeground }}>{title}</Text>
          {children}
        </ScrollView>
      </View>
    </View>
...
export function MenuRow({ label, description, leading, selected, disabled, destructive, onPress }: {
  ...
  return <Pressable accessibilityRole={selected === undefined ? 'button' : 'radio'}
