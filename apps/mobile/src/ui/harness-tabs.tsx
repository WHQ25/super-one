...
export function HarnessTabs(props: {
  ...
  // Keep the selected tab taller than its track, as on desktop. The wrapper
  // leaves room for a 44-point touch target without enlarging the visible tabs.
  const slotStyle = (active: boolean) => ({
    minHeight: 32, justifyContent: 'center' as const, alignItems: 'center' as const,
    flexDirection: 'row' as const, gap: 4, paddingHorizontal: 14, borderRadius: radius.md,
    borderWidth: 1, borderColor: active ? colors.border : 'transparent',
    backgroundColor: active ? colors.background : 'transparent',
  })
  return <>
    <View style={{ flexDirection: 'row', paddingHorizontal: 2, paddingVertical: 6, gap: 2 }}>
      <View pointerEvents="none" style={{ position: 'absolute', left: 2, right: 2, top: 8, bottom: 8,
      ...
      <Pressable accessibilityRole="tab" accessibilityState={{ selected: !menuActive, disabled: props.disabled }}
        accessibilityLabel={fixed.label} disabled={props.disabled} hitSlop={{ top: 6, bottom: 6 }}
      ...
      {menuTab ? <Pressable ref={menu.ref} accessibilityRole="tab"
        ...
        accessibilityLabel={menuTab.label} disabled={props.disabled} hitSlop={{ top: 6, bottom: 6 }}
        ...
        {slots.menu.length > 1 ? <RotatingChevron open={!!menu.anchor} size={14} color={colors.mutedForeground} /> : null}
      </Pressable> : null}
    </View>
