...
/**
 * What `/mcp` shows on a phone: which servers this session is running and
 * whether they are up.
 *
 * The desktop popup can reconnect a server and start an OAuth flow. A phone
 * cannot finish one, and a button that begins something it cannot complete is
 * worse than a readout — so a server needing sign-in says where to do it.
 */
export function McpSheet({ visible, servers, loading, error, onDismiss }: {
  visible: boolean
  ...
  return <Sheet visible={visible} title="MCP servers" icon={Plug} onDismiss={onDismiss}>
    <ScrollView testID="mcp-sheet" style={{ maxHeight: 420 }} contentContainerStyle={{ padding: 12, gap: 2 }}>
      ...
      : !servers.length ? <Text style={{ padding: 8, color: colors.mutedForeground, fontSize: 13 }}>
        ...
        style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 8, minHeight: 44 }}>
