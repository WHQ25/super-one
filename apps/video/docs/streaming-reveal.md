# Streaming reveal design reference

Read when changing assistant streaming animation. Keep existing public timing
props compatible; exact chunk sizes and pause distributions are design choices.
...
## Current baseline (checked September 14, 2026)

`computeReveal` in `packages/desktop-mocks/src/desktop/chat-mock.tsx` still uses
character-count interpolation. The schedule below is a proposed approach for a
streaming-reveal task, not a claim that token chunking already exists or a
requirement to refactor it while editing unrelated scenes.
...
2. **Schedule each token's arrival time** deterministically from `(messageId, tokenIndex)` so it's stable across frames and renders (Remotion requires determinism). Around a mean inter-token delay derived from `typingCps`, add per-token jitter and occasional long pauses:
   ...
   - For example, every ~12–20 tokens, inject a longer pause (e.g. `meanMs * 3–5`) to mimic the model "thinking" before a sentence boundary.
