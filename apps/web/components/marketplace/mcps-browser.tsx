...
import { listMcps, type McpListResponse } from "@/lib/marketplace/api"
...
export function McpsBrowser({
  ...
  const [data, setData] = useState<McpListResponse>(initial)
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      setLoading(true)
      listMcps({ category, q: query }, { signal: controller.signal })
    ...
    <div className="mx-auto w-full max-w-6xl px-4 sm:px-6">
      ...
      <section className="grid gap-8 md:grid-cols-[200px_1fr]">
        <CategoryRail
          ...
          options={options}
          value={category}
          onChange={(next) => setCategory(next as McpCategory | "all")}
        />
        <div>
          {data.items.length === 0 ? (
            ...
            >
              {data.items.map((entry) => (
                <McpCard key={entry.slug} entry={entry} locale={locale} />
              ))}
            </div>
          )}
        </div>
