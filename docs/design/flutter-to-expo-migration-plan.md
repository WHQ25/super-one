...
**Wave 8 execution update (2026-09-04):** WP-25 hygiene, logical commits, rebase, package tests, changed-Desktop verification, and workspace typecheck are complete on `feat/migrate-to-expo`; the final merge to `main` remains. WP-28 is complete: Desktop and Expo share every planned non-mini-app specialised presenter, including Codex collaboration, plan/insight/media generation, Browser/WebMCP/download, Device/Computer Use/screenshots, Session Collaboration/Agents/Findings, Session Archive, Automation, Config Apply, and Media Providers. Desktop keeps only host actions and live-store adapters at the old paths. Remote transcript metadata is privacy-projected; typed text, selectors, coordinates, refs, URLs, page-tool arguments, app identifiers, collaboration credentials, agent prompts, automation prompts, config values, search queries, and session ids remain stripped. Sanitized offline recordings cover these families, and chat-view has 40 passing Playwright scenarios. Mini-app iframes remain deferred; WP-26/27 shell and navigation parity are next.
...
*End of plan. WP-01–22 and WP-28 are complete; WP-23/24 were superseded by Wave 8. **Current: finish the WP-25 merge, then complete WP-26 design-system/navigation foundations and WP-27 shell parity.** WP-29 follows. Mini-app iframes remain deferred; physical-device evidence is no longer a gate, and the one-smoke-per-platform list lives in WP-29.*
