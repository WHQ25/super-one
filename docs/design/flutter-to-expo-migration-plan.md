...
**Wave 8 execution update (2026-09-04):** WP-25 hygiene, logical commits, rebase, package tests, changed-Desktop verification, and workspace typecheck are complete on `feat/migrate-to-expo`; the final merge to `main` remains. WP-26 and WP-28 are complete. The RN shell now uses generated desktop-semantic light/dark tokens, a native navigation stack, shared primitives, and the same theme payload for chat and terminal WebViews; Desktop and Expo share every planned non-mini-app specialised presenter. Mini-app iframes remain deferred. **WP-27 shell parity is current**, followed by WP-29 and the final WP-25 merge.
...
*End of plan. WP-01–22, WP-26, and WP-28 are complete; WP-23/24 were superseded by Wave 8. **Current: complete WP-27 shell parity.** WP-29 and the final WP-25 merge follow. Mini-app iframes remain deferred; physical-device evidence is no longer a gate, and the one-smoke-per-platform list lives in WP-29.*
