...
**Completed 2026-09-04.** Generated RN colour tokens now come from the desktop OKLch source plus all six harness brand accents; system appearance drives native chrome and both WebViews. `App.tsx` is a 10-line root over a native-stack shell with phone and 768 px master/detail routing. Pairings, projects, sessions, shared primitives, and light/dark simulator rendering are in place; the old `styles.ts` is removed and `src/**/*.tsx` contains no raw hex colours.
...
installed and enforced by the release-config preflight. The Expo project is linked,
its generated project id and Update URL are checked statically, and EAS owns the
Android signing key. Internal Android build 3 completed and launched on the built-in
Android 16 device; camera permission plus QR scanner open/cancel passed. Signed iOS
production build 4 completed; TestFlight submission, physical-device distribution and
smoke, and Flutter archival remain pending. The first
...
**2026-09-04 (WP-19 reconnect hardening):** replaced the one-shot reconnect attempt with a single cancellable loop using the Flutter-proven 1/2/4/8/16/30-second bounded backoff. A successful socket open no longer clears the offline banner early: the connection becomes `connected` only after the active session completes buffer-first restore and exposes its released epoch. Initial session open and server-reset rehydrate also synchronize that runtime epoch, and a restarted Chat WebView receives the current connection snapshot on `ready`. Switching pairings cancels the old loop and disposes its runtimes so late events cannot enter the prior session. Controller and Relay lifecycle regressions prove retry, cancellation, and restore ordering. The physical network-flap gate remains pending for manual testing.
...
**2026-09-04 (WP-28 session/agent presenters):** moved Session Collaboration, List Agents, and Report Findings into `@superone/chat-view` while retaining the Desktop component paths as host adapters. Expo now renders collaboration Markdown, agent rosters, and review findings with the same presenters; file findings route through the native `openFile` action, while session-title navigation remains owned by WP-27. Remote transcript projection keeps visible collaboration content but strips credentials, peer tokens, and agent prompts. Chat-view has 38 passing offline Playwright scenarios; Session Archive, Automation, Config, and Media Providers remain.
...
**2026-09-04 (WP-27/29 file and terminal smoke):** a real `mobile_share_file` call delivered `apps/mobile/package.json` into the native Android inbox with the expected name, project path, JSON MIME type, 2.1 KB size, and Ready state. The terminal then opened the session-owned shell and returned the exact worktree path from `pwd`. This run exposed a fast-submit race where React state lag could send only the first character of pasted terminal input; submission now uses `TextInput`'s native event text as the authoritative line. Mobile's 75 tests and typecheck remain green after the fix. These built-in-device checks supplement rather than replace the required physical-platform smoke.

**2026-09-04 (WP-19 Flutter recovery parity):** ported the already validated Flutter recovery semantics without weakening Expo's buffer-first restore. App foregrounding now forces an immediate reconnect, subsequent failures use the 1/2/4/8/16/30-second capped schedule, relay desktop peer loss waits for its returning handshake before rehydrating, and desktop shutdown/revocation cancels reconnect completely. Relay and LAN sockets register the mobile identity before replay or commands, fixing the manual-LAN first-command rejection. A restored chat now resubscribes the current live terminal or recreates an interrupted terminal create. Relay-client has 60 passing tests, Mobile has 80 across 29 files, and both typechecks are green. The 10-second physical airplane-mode check remains intentionally manual.
...
service. Submission `e9fd75c9-1ec2-4eb7-9fe7-c9f46d85a4a8` uploaded build 21
successfully. App Store Connect reports `VALID` / `READY_FOR_BETA_TESTING`; assigning
the build to internal testers, physical iPhone/Android smoke, Flutter archival, and
the final merge remain pending.

**2026-09-04 (WP-25 latest-main verification):** rebased the migration branch onto
the latest three `main` commits without conflicts. The new Codex realtime ordering
change is Desktop-only because Expo does not expose that voice surface; shared reducer
and presenter behavior did not diverge. The first 340-file Desktop run exposed an
existing three-second PTY assertion window under parallel load; replacing it with a
bounded condition wait made both the focused test and the repeated changed-main run
...
*End of plan. WP-01–22 and WP-26–28 are complete; WP-23/24 were superseded by Wave 8. **Current: complete WP-29 lean release, then the final WP-25 merge.** Mini-app iframes remain deferred; the one-smoke-per-platform list lives in WP-29.*
