...
export interface ReportFindingsToolBlockPresenterProps {
  ...
  allowExpand?: boolean
  renderFile?: (finding: ReviewFinding) => ReactNode
}

function defaultFile(finding: ReviewFinding) {
  const title = finding.line != null ? `${finding.file}:${finding.line}` : finding.file
  return (
    <span className="max-w-56 truncate rounded bg-muted px-1.5 py-0.5 font-mono text-xs text-muted-foreground" title={title}>
 ...
 * behind a chevron.
 */
export function ReportFindingsToolBlockPresenter({
  params,
  isStreaming,
  isError,
  isDenied,
