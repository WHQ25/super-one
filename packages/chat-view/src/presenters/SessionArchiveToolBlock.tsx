...
export interface SessionArchiveToolBlockPresenterProps {
...
function HarnessGlyph({
  ...
  const icon = renderHarnessIcon?.(harness, acpAgentId)
  if (!icon) {
  ...
  return (
    <button
      ...
      onClick={(e) => {
        e.preventDefault()
        e.stopPropagation()
        void onOpenSession(sessionId, projectId)
      }}
      className={cn(
...
export function SessionArchiveToolBlockPresenter({
  toolName,
  params,
  result,
  isStreaming = false,
