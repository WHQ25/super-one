...
export interface SessionCollabToolBlockPresenterProps {
  toolName: string
...
export function SessionCollabToolBlockPresenter({
  toolName,
  params,
  result,
  isStreaming,
  ...
  onOpenSession,
  renderMarkdown,
}: SessionCollabToolBlockPresenterProps) {
  const { t } = useTranslation()
  ...
  const collabTone: ToolRowTone = isDenied || status === 'rejected' || status === 'cancelled'
    ? 'denied'
  ...
  const header = (
    <>
      ...
      />
      <ToolName streaming={isStreaming && !isDenied} tone={collabTone}>
        {label}{streamingDots}
      </ToolName>
      {summaryPeer && (
        <SessionTitleLink
            ...
            <div className="space-y-2 border-t border-border/40 px-2 py-2 text-xs">
              {inboxMessages.map((msg, index) => (
                ...
                >
                  ...
                  {msg.content ? (
                    <CollabMessageBody content={msg.content} renderMarkdown={renderMarkdown} />
                  ) : null}
                </div>
              ))}
            </div>
