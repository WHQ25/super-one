...
export function resolveProjectFileHref(
  ...
  // An in-document anchor — `[Setup](#setup)`, the heading links a table of
  // contents is made of. `new URL('#setup')` throws, so without this it falls
  // through to the project-relative branch and becomes `<root>/#setup`: a chip
  // naming a file that cannot exist. The text-level rewrite in
  // `resolveMarkdownFileLinks` already skips `#`-leading destinations, so this
  // is what makes the two passes agree.
