...
export function CodexSessionIcon({ status, active = false, size }: SessionIconProps) {
      ...
      >
        <defs>
          ...
          <style>
            {`
              ...
              @keyframes codex-v5-cursor-default {
                0%, 48%, 100% { opacity: 1; }
                50%, 98% { opacity: 0; }
              }
              @keyframes codex-v5-cursor-fast {
                0%, 48%, 100% { opacity: 1; }
                50%, 98% { opacity: 0; }
              }
              @keyframes codex-v5-bg-breath {
                0%, 100% { opacity: 0.25; }
                50% { opacity: 0.95; }
              }
            `}
        ...
        {status === 'default' && (
          <g transform="translate(12 12)">
            <g style={{ transformOrigin: 'center center', animation: 'codex-v5-scale 3.2s ease-in-out infinite' }}>
              <g transform="translate(-12 -12)">
                <path
                  d={cloudPath}
                  fill={`url(#${baseGradId})`}
                ...
                <path
                  ...
                  fill="#fff"
                  style={{ animation: 'codex-v5-cursor-default 1.2s step-end infinite' }}
                />
              </g>
            </g>
          </g>
        ...
        {status === 'running' && (
          <>
            ...
            <path
              d="M12.546 13.909a.637.637 0 000 1.272h3.636a.637.637 0 100-1.272h-3.636z"
              fill="#fff"
              style={{ animation: 'codex-v5-cursor-fast 0.4s step-end infinite' }}
            />
          </>
        )}

        {status === 'background' && (
          <g style={{ animation: 'codex-v5-bg-breath 2.5s ease-in-out infinite' }}>
            <g transform="translate(12 12)">
              <g style={{ transformOrigin: 'center center', animation: 'codex-v5-scale 3.2s ease-in-out infinite' }}>
                <g transform="translate(-12 -12)">
                  <path
