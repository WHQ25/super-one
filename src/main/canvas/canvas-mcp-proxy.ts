...
let mcpConfig: McpSdkServerConfigWithInstance | null = null
...
export function initCanvasMcpProxy(windowGetter: () => BrowserWindow | null): void {
...
export function getCanvasMcpProxy(): McpSdkServerConfigWithInstance {
  if (!mcpConfig) {
    ...
    mcpServer = mcpConfig.instance as unknown as McpServer
  }
  return mcpConfig
}
...
export function registerAppTools(appId: string, tools: MiniAppToolDefinition[]): void {
  ...
  for (const tool of tools) {
    ...
    log.info('[canvas-mcp] registered tool: %s', namespacedName)
  }
...
export function unregisterAppTools(appId: string): void {
  ...
  for (const [name, tool] of registeredTools) {
    if (name.startsWith(prefix)) {
      ...
      log.info('[canvas-mcp] unregistered tool: %s', name)
    }
  }
...
export function notifyAppReady(appId: string): void {
  ...
  if (entry?.resolve) {
    ...
    log.info('[canvas-mcp] app ready: %s (%dms)', appId, elapsed)
    appReadyGates.delete(appId)
    entry.resolve()
  } else {
    log.info('[canvas-mcp] app ready (early): %s', appId)
