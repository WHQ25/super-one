...
const MODEL_ENV_KEY_SET: Set<string> = new Set(MODEL_ENV_KEYS.map((m) => m.key))
const INTERNAL_ENV_KEYS: Set<string> = new Set(['ANTHROPIC_AUTH_TOKEN', 'ANTHROPIC_API_KEY'])

function splitEnv(envJson: string): { modelEnv: Record<string, string>; internalEnv: Record<string, string>; restEnv: string } {
  try {
...
function mergeEnv(restEnv: string, modelEnv: Record<string, string>, internalEnv: Record<string, string>): string {
  try {
