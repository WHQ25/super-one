...
import type { GitLogEntry, CodexReviewTarget } from '../../../../shared/agent-types'
...
function HighlightText({ text, indices }: { text: string; indices: number[] }) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (isMatch !== inHighlight) {
      if (run) parts.push(inHighlight ? <mark key={i} className="bg-transparent text-orange-400 font-semibold">{run}</mark> : run)
      run = ''
      inHighlight = isMatch
    }
    run += text[i]
  }
  if (run) parts.push(inHighlight ? <mark key="end" className="bg-transparent text-orange-400 font-semibold">{run}</mark> : run)
  return <>{parts}</>
}
            ...
            <div ref={commitListRef} className="max-h-40 overflow-y-auto rounded-md border border-border">
              {loading ? (
                ...
                filteredCommits.map((commit, i) => (
                  ...
                  >
                    <code className="shrink-0 font-mono text-blue-400"><HighlightText text={commit.sha.slice(0, 7)} indices={commit.shaIndices.filter((i) => i < 7)} /></code>
